...
class TestFeatureConfig:
    ...
    def test_invalid_calendar_feature(self):
        ...
        with pytest.raises(ValueError, match="Invalid calendar features"):
            FeatureConfig(calendar_features=["invalid_feature"])
    ...
    def test_invalid_rolling_aggregation(self):
        ...
        with pytest.raises(ValueError, match="Invalid aggregations"):
            FeatureConfig(rolling_windows={7: ["invalid_agg"]})

    def test_invalid_window_size(self):
        """Test that invalid window sizes raise ValueError."""
        with pytest.raises(ValueError, match="positive integers"):
            FeatureConfig(rolling_windows={0: ["mean"]})
...
class TestComputeFeatureHash:
    ...
    def test_deterministic_hash(self):
        ...
        config1 = FeatureConfig(lags=[1, 7], calendar_features=["dayofweek"])
        config2 = FeatureConfig(lags=[1, 7], calendar_features=["dayofweek"])
        assert compute_feature_hash(config1) == compute_feature_hash(config2)

    def test_different_configs_different_hashes(self):
        """Test that different configs produce different hashes."""
        config1 = FeatureConfig(lags=[1, 7])
        config2 = FeatureConfig(lags=[1, 14])
        assert compute_feature_hash(config1) != compute_feature_hash(config2)

    def test_order_independence(self):
        """Test that hash is independent of order in lists."""
        config1 = FeatureConfig(lags=[1, 7, 14], calendar_features=["month", "dayofweek"])
        config2 = FeatureConfig(lags=[14, 1, 7], calendar_features=["dayofweek", "month"])
        assert compute_feature_hash(config1) == compute_feature_hash(config2)
...
class TestConfigsEqual:
    ...
    def test_equal_configs(self):
        ...
        config1 = FeatureConfig(lags=[1, 7])
        config2 = FeatureConfig(lags=[1, 7])
