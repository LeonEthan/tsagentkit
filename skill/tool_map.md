...
- `ensemble(predictions, method, quantiles)` - Aggregate predictions into one DataFrame
...
### Length Utilities
- `check_data_compatibility(spec, context_length, prediction_length)` - Check compatibility
- `get_effective_limits(spec, loaded_model=None)` - Get context/prediction limits
- `adjust_context_length(context, spec, pad_value=None)` - Adjust context array length
- `validate_prediction_length(h, spec, strict=False)` - Validate or clip horizon
...
| Ensemble | `ensemble(predictions, method, quantiles)` | Returns ensemble DataFrame |
